...
const resetMocks = () => {
  ...
  mockTipoTransacoes = JSON.parse(
    JSON.stringify(initialMockState.tipoTransacoes)
  );
  nextTipoTransacaoId =
    mockTipoTransacoes.length > 0
      ? Math.max(...mockTipoTransacoes.map((t) => t.id)) + 1
  ...
  transacoes: {
    ...
    create: async (data: any) => {
      const dataToCreate = {
        ...
        valor:
          typeof data.valor === "string" ? parseFloat(data.valor) : data.valor,
    ...
    update: async ({ where, data }: { where: { id: number }; data: any }) => {
      ...
      if (index > -1) {
        ...
        if (data.data !== undefined)
          updatedTransaction.data = new Date(data.data);
    ...
    delete: async ({ where }: { where: { id: number } }) => {
      ...
      if (deleted) {
        ...
        console.log(
          `Mock: Transação ${where.id} não encontrada para exclusão.`
        );
      }
      return deleted;
    },
