...
const mockPrisma = {
  ...
  tipoTransacoes: {
    // Novo mock para TipoTransacoes (camelCase)
  ...
  categorias: {
    // O nome aqui é 'categorias' (plural)
    findMany: async ({ orderBy }: { orderBy?: { nome?: "asc" | "desc" } }) => {
  ...
  transacoes: {
    // Nome 'transacoes' (plural), conforme seu `@@map("transacoes")` e convenção
